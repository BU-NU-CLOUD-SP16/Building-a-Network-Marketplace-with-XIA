		...
		u64 key_attrs, u64 mask_attrs, bool log)
{
	...
	if (match->key->eth.type == htons(ETH_P_XIP)) {
		key_expected |= 1 << OVS_KEY_ATTR_XIA;
		if (match->mask && (match->mask->key.eth.type == htons(0xffff)))
			mask_allowed |= 1 << OVS_KEY_ATTR_XIA;

		if (match->mask && (match->mask->key.xip.xia_version == 0xff))
			mask_allowed |= 1 << OVS_KEY_ATTR_XIA;
	}
...
static const struct ovs_len_tbl ovs_tunnel_key_lens[OVS_TUNNEL_KEY_ATTR_MAX + 1] = {
	...
	[OVS_TUNNEL_KEY_ATTR_VXLAN_OPTS]    = { .len = OVS_ATTR_NESTED, .next = ovs_vxlan_ext_key_lens },
						...
						[OVS_KEY_ATTR_ETHERTYPE] = { .len = sizeof(__be16) },
							[OVS_KEY_ATTR_IPV4]	 = { .len = sizeof(struct ovs_key_ipv4) },
								[OVS_KEY_ATTR_IPV6]	 = { .len = sizeof(struct ovs_key_ipv6) },
									...
									[OVS_KEY_ATTR_XIA]	 = { .len = sizeof(struct ovs_key_xia) },
		...
		bool is_mask, bool log)
{
	...
	// ADD support for XIA
	if (attrs & (1 << OVS_KEY_ATTR_XIA)) {
		const struct ovs_key_xia *xia_key;

		xia_key = nla_data(a[OVS_KEY_ATTR_XIA]);
		if (!is_mask && xia_key->xia_version != 1) {
			OVS_NLERR(log, "XIA version %d is out of range",
					xia_key->xia_version);
			return -EINVAL;
		}
		...
		struct sk_buff *skb)
{
	...
	} else if (swkey->eth.type == htons(ETH_P_XIP)) { // add support for XIA
		struct ovs_key_xia *xia_key;

		nla = nla_reserve(skb, OVS_KEY_ATTR_XIA, sizeof(*xia_key));
		if (!nla)
			goto nla_put_failure;
		...
		bool *skip_copy, __be16 eth_type, bool masked, bool log)
{
	...
	switch (key_type) {
		...
		*skip_copy = true;
		...
		if (eth_type != htons(ETH_P_XIP))
			return -EINVAL;

		xia_key = nla_data(ovs_key);

		if (masked) {
			const struct ovs_key_xia *mask = xia_key + 1;

			/* Non-writeable fields. */
			if (mask->xia_version)
				return -EINVAL;
		} else {
			if (xia_key->xia_version != flow_key->xip.xia_version)
				return -EINVAL;
		}
		break;
